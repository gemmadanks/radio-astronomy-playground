...
def test_skymodel_config_valid():
    cfg = SkyModelConfig(
        name="toy", num_sources=5, max_flux_jy=1.0, fov_deg=1.0, seed=0
    )
...
def test_skymodel_config_rejects_bad_num_sources():
    with pytest.raises(ValidationError):
        SkyModelConfig(name="toy", num_sources=0, max_flux_jy=1.0, fov_deg=1.0, seed=0)
...
def test_skymodel_config_rejects_bad_fov(bad_fov):
    with pytest.raises(ValidationError):
        SkyModelConfig(
            name="toy", num_sources=5, max_flux_jy=1.0, fov_deg=bad_fov, seed=0
        )
...
def test_skymodel_config_rejects_bad_max_flux_jy(bad_max_flux_jy):
    with pytest.raises(ValidationError):
        SkyModelConfig(
            name="toy", num_sources=5, max_flux_jy=bad_max_flux_jy, fov_deg=1.0, seed=0
        )
