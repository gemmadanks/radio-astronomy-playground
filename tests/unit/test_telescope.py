...
def test_telescope_array(name, num_antennas, diameter, seed):
    """Test that array returns a numpy array of antenna positions."""
    telescope = Telescope(
        name=name, num_antennas=num_antennas, diameter=diameter, seed=seed
    )
    assert telescope.rng is not None
    assert telescope.array.shape == (num_antennas, 3)
...
def test_telescope_array_determinism():
    ...
    telescope1 = Telescope(
        name="DeterministicArray", num_antennas=50, diameter=30.0, seed=123
    )
...
def test_telescope_array_variability():
    ...
    telescope1 = Telescope(
        name="VariableArray", num_antennas=50, diameter=30.0, seed=123
    )
    telescope2 = Telescope(
        name="VariableArray", num_antennas=50, diameter=30.0, seed=456
    )
...
def test_telescope_plot():
    ...
    telescope = Telescope(name="PlotArray", num_antennas=20, diameter=50.0, seed=42)
    telescope.plot()


def test_telescope_reconfigure():
    """Test that reconfigure changes the array configuration."""
    telescope = Telescope(name="ReconfigArray", num_antennas=30, diameter=40.0, seed=42)
